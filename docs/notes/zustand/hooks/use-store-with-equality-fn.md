...
description: 如何在 React 中有效使用 vanilla stores
createTime: 2025/06/15 17:52:18
permalink: /zustand/p07uxk8a/
---

`useStoreWithEqualityFn` 是一个 React Hook，让你在 React 中使用 vanilla store，就像 `useStore` 一样。但是，它提供了一种定义自定义等价检查的方法。这允许对组件何时重新渲染进行更细粒度的控制，提高性能和响应性。

> [!IMPORTANT]
> 为了使用 `zustand/traditional` 中的 `useStoreWithEqualityFn`，你需要安装 `use-sync-external-store` 库，因为 `zustand/traditional` 依赖于 `useSyncExternalStoreWithSelector`。
...
- [用法](#用法)
  ...
  - [在 React 中使用动态 vanilla stores](#在-react-中使用动态-vanilla-stores)
...
- `equalityFn`: 一个让你跳过重新渲染的函数。

#### 返回值

`useStoreWithEqualityFn` 根据选择器函数返回基于当前状态的任何数据，并让你使用等价函数跳过重新渲染。它应该接受一个 store、一个选择器函数和一个等价函数作为参数。
...
const useCounterStore = <U,>(
  ...
  return useStoreWithEqualityFn(getOrCreateCounterStoreByKey(key), selector)
}
```
